    ...
    for k in range(0,self.NR):
      for k1 in range(0,len(self.labels)):
        for k2 in range(0,len(self.labels)):
          ...
          sro_real[kp][k]=-(sro_real[kp][k]-self.c[k1]*self.c[k2])/(self.c[k1]*self.c[k2]) #/Warren_Cowley_factor 
          #sro_real[kp][k]=(sro_real[kp][k]-self.c[k1]*self.c[k2])/Warren_Cowley_factor
    self.sro_real=sro_real
    print("SRO in real space: ",self.sro_real)
  def cal_spin(self):
  ...
  def sro(self,pair):
    ...
    for ix in range(0,self.N[0]):
      ...
      for iy in range(0,self.N[1]):
        ...
        # for (1-10) plane
        #k=np.array([4.0*ix/(self.N[0]-1),4.0*iy/(self.N[0]-1),4.0*iy/(self.N[1]-1)])
        # for (112) plane
        k=np.array([2.0*(-2*ix-iy)/(self.N[0]-1),2.0*(2*ix-iy)/(self.N[0]-1),2.0*iy/(self.N[1]-1)])
        alpha_k_list[ix].append(self.sro_k(k,pair))
     # if iy==19 or iy==39: print sro_k(atoms_pos,k,c)
      alpha_k_list[ix].pop(0)
    return alpha_k_list
  def plot_sro(self,solute_pair,new_labels,color_min,color_max):
   ...
   # print Z
    plt.contour(X,Y,Z,80,cmap=cm.coolwarm, vmin=color_min, vmax=color_max)
...
def plt_sro_k(rfile,sro_source):
  ...
  for i in range(0,len(labels)):
    for j in range(0,len(labels)):
      ...
      if 1==1: #j>=i:
        fig_id=i*3+j+1 # +=1
        plt.subplot(len(labels),3,fig_id)
        solute_pair=np.array([i,j])
        case.plot_sro(solute_pair,new_labels,-2,2) #-22,73)
  #plt.subplot(3,3,3)
  #case.plot_sro(np.array([0,1]),str(labels[0]+"-"+labels[1]),-10,18)
  #plt.subplot(3,3,6)
  #case.plot_sro(np.array([1,2]),str(labels[1]+"-"+labels[2]),-10,18)
  #plt.subplot(3,3,9)
  #case.plot_sro(np.array([0,2]),str(labels[0]+"-"+labels[2]),-22,73)
